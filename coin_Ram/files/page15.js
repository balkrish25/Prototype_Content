...
boxes.forEach(box => {
  ...
  box.addEventListener("drop", e => {
    ...
    if (coin && coin.alt.includes("1 Rupee")) {
...
function checkCompletion() {
  const filled = Array.from(document.querySelectorAll(".drop-box")).every(box => box.children.length > 0);
  if (filled) {
    setTimeout(() => {
      window.location.href = "/files/page16.html";
    }, 800);
  }
}
